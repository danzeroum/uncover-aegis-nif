defmodule UncoverAegisWeb.InsightsLive do
  ...
  - **Assincrono por design**: `handle_event/3` nao chama o LLM/banco
    ...
    do usuario). O processamento ocorre em `handle_info/2`.

  - **PubSub real-time**: ao montar, o LiveView assina o topico
    `"anomalies"`. Quando o MVP3 (CampaignMonitor) detecta uma anomalia,
    faz broadcast neste topico e o alerta aparece instantaneamente
    na tela do usuario — sem polling, sem reload.

  - **Metadados visuais**: cada resposta do assistente exibe o SQL
    executado, o tempo de validacao Rust (microsegundos) e o tempo
    de query (milissegundos), demonstrando observabilidade.
  """
    ...
    <div class="flex flex-col h-full">
      ...
      <div class="flex-1 overflow-y-auto p-4">
        ...
        >
          ...
          <h2 class="text-lg font-semibold text-gray-700">Pronto para analisar suas campanhas</h2>
          <p class="text-sm text-gray-500 mt-2 max-w-sm">
            Faça perguntas em português sobre seus dados de marketing.
            Cada resposta é validada pelo escudo Rust antes de tocar o banco.
          </p>
      ...
      <div class="px-4 pb-4 pt-2 bg-white border-t border-gray-100">
        <form phx-submit="send_message" class="flex gap-2">
          <input
            ...
            name="message"
            value={@current_input}
            placeholder="ex: qual o gasto total?"
            autocomplete="off"
            disabled={@loading}
            class="flex-1 rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
          />
          <button
            type="submit"
            disabled={@loading}
            class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-lg text-sm font-medium transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <%= if @loading, do: "...", else: "Perguntar" %>
          </button>
        </form>
