defmodule UncoverAegisWeb.InsightsLive do
  ...
  alias UncoverAegis.{Insights, Repo, Native}
  import Ecto.Query
  ...
  # Paleta de cores por plataforma para o Adstock
  @platform_colors %{
    "google"   => "rgba(59,130,246,0.8)",
    "meta"     => "rgba(99,102,241,0.8)",
    "tiktok"   => "rgba(236,72,153,0.8)",
    "linkedin" => "rgba(14,165,233,0.8)"
  }
  ...
  # MMM: muda decay via slider
  def handle_event("mmm_decay", %{"value" => v}, socket) do
    decay = parse_slider(v, 0.7)
    {:noreply, assign(socket, mmm_decay: decay, mmm_result: nil)}
  end

  # MMM: muda alpha via slider
  def handle_event("mmm_alpha", %{"value" => v}, socket) do
    alpha = parse_slider(v, 2.0)
    {:noreply, assign(socket, mmm_alpha: alpha, mmm_result: nil)}
  end

  # MMM: calcula adstock
  def handle_event("mmm_calculate", _params, socket) do
    send(self(), {:run_adstock,
      socket.assigns.mmm_selected,
      socket.assigns.mmm_decay,
      socket.assigns.mmm_alpha
    })
  ...
  def handle_info({:run_adstock, campaign_id, decay, alpha}, socket) do
      ...
      case Native.calculate_adstock(spends, decay, alpha, half_sat) do
        {:ok, r} ->
          %{
            ...
            dates: dates,
            spends: spends,
            adstock_values: r.adstock_values,
            saturated_values: r.saturated_values,
    ...
    socket =
      socket
      |> assign(mmm_result: result, mmm_loading: false)
      |> maybe_push_chart_event(result, dates)
    ...
    <div class="flex flex-col h-full">
      ...
      <div class="px-6 py-4 border-b border-gray-200 bg-white shadow-sm">
        ...
        <div class="flex gap-1 mt-4 border-b border-gray-100">
          <button phx-click="switch_tab" phx-value-tab="insights" class={tab_class(@active_tab == :insights)}>
            \u{1F4CA} Insights
          </button>
          <button phx-click="switch_tab" phx-value-tab="campaigns" class={tab_class(@active_tab == :campaigns)}>
            \u{1F4CB} Campanhas
          </button>
          <button phx-click="switch_tab" phx-value-tab="mmm" class={tab_class(@active_tab == :mmm)}>
            \u{1F9EA} Mix de Mídia
          </button>
                ...
                <%= if @sql_mode do %>
                  <button :for={q <- @example_sqls} phx-click="use_example" phx-value-question={q}
                    class={["text-xs border rounded-full px-3 py-1.5 transition font-mono",
                      if(String.starts_with?(q, "SELECT"), do: "bg-white border-gray-200 hover:border-blue-400 hover:text-blue-600 text-gray-600", else: "bg-red-50 border-red-200 hover:border-red-400 text-red-600")]}>
                    <%= q %>
                  </button>
                <% else %>
        ...
        <div :if={@active_tab == :mmm} class="p-6">
          ...
          <div class="bg-white rounded-xl border border-gray-200 p-5 mb-6 shadow-sm">
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-3">
              ...
              <div>
                ...
                <div class="flex flex-col gap-1">
                  <button
                    :for={c <- @mmm_campaigns}
                    phx-click="mmm_select_campaign" phx-value-campaign={c}
                    class={["text-xs px-3 py-1.5 rounded-lg border text-left transition",
                      if(@mmm_selected == c,
                        do: "bg-blue-600 border-blue-600 text-white font-semibold",
              ...
              <div>
                ...
                <p class="text-xs text-gray-400 mb-3">
                  Quanto do efeito de hoje persiste amanhã.
                  <strong>0.7 = digital</strong> | 0.9 = TV/offline
                </p>
                <input
                  type="range" min="0" max="1" step="0.05"
                  value={@mmm_decay}
                  phx-change="mmm_decay"
                  name="value"
                  class="w-full accent-blue-600"
                />
                <div class="flex justify-between text-xs text-gray-400 mt-1">
                  <span>0 (sem memória)</span>
                  <span class="font-bold text-blue-600"><%= @mmm_decay %></span>
                  <span>1.0 (memória total)</span>
                </div>
              </div>
              ...
              <div>
                <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1">
                  Curva de Saturação (α Hill)
                </label>
                <p class="text-xs text-gray-400 mb-3">
                  Velocidade dos retornos decrescentes.
                  <strong>α=2 = curva em S</strong> (recomendado digital)
                </p>
                <input
                  type="range" min="0.5" max="5" step="0.5"
                  value={@mmm_alpha}
                  phx-change="mmm_alpha"
                  name="value"
                  class="w-full accent-green-600"
                />
                <div class="flex justify-between text-xs text-gray-400 mt-1">
                  <span>0.5 (leve)</span>
                  <span class="font-bold text-green-600">α = <%= @mmm_alpha %></span>
            ...
            <div class="mt-5 flex justify-end">
              <button
                phx-click="mmm_calculate"
                disabled={@mmm_loading or is_nil(@mmm_selected)}
          ...
          <div :if={@mmm_result}>
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-5">
              ...
              <p class="text-sm text-blue-700">
                A campanha <strong><%= @mmm_result.campaign_id %></strong> gerou um impacto acumulado
                (Adstock) de <strong>R$ <%= format_brl(List.last(@mmm_result.adstock_values)) %></strong>
                no último período, contra um gasto real de
                <strong>R$ <%= format_brl(List.last(@mmm_result.spends)) %></strong>.
                O período de maior contribuição respondeu por
                <strong><%= @mmm_result.contribution_pct |> Enum.max() |> :erlang.float_to_binary(decimals: 1) %>%</strong>
                do impacto total saturado.
              </p>
            </div>
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                ...
                <div class="h-56">
                  <canvas id="adstock-bar" phx-hook="AdstockBar"></canvas>
                </div>
              </div>
              <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                <h3 class="text-sm font-semibold text-gray-700 mb-3">Contribuição por Período</h3>
                <div class="h-56">
                  <canvas id="contrib-donut" phx-hook="ContribDonut"></canvas>
                </div>
              </div>
            </div>
              ...
              <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th class="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Satuação</th>
            ...
            <p class="text-xs text-gray-400 mt-3 text-center">
              Adstock = gasto acumulado com carry-over (decay=<%= @mmm_result.decay %>) &nbsp;|&nbsp;
              Saturação Hill α=<%= @mmm_result.alpha %> &nbsp;|&nbsp;
              K=R$ <%= format_brl(@mmm_result.half_saturation) %>
            </p>
          </div>
          ...
          <div :if={is_nil(@mmm_result) and not @mmm_loading} class="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p class="text-sm text-gray-500 mt-2 max-w-md">
              ...
              Ajuste o <strong>carry-over</strong> e a <strong>curva de saturação</strong> para simular diferentes cenários de mídia.
            </p>
          </div>
        </div>
        ...
        <div :if={@active_tab == :sentinel} class="p-6">
          ...
          <div class="mt-8 bg-blue-50 border border-blue-200 rounded-xl p-5">
            ...
            <p class="text-sm text-blue-700">
              ...
              Um valor acima de <strong>3.0</strong> significa que o gasto de hoje é estatisticamente incomum —
              pode indicar bug na integração, fraude de clique, ou oportunidade de escala.
            </p>
  ...
  defp format_success(result) do
    ...
    anomaly_note = if result[:anomaly], do: "\n\n\u26A0\uFE0F Anomalia (Z-Score: #{Float.round(result.z_score, 2)})", else: ""
    %{
      role: :assistant,
  ...
  defp tab_class(true),  do: "px-4 py-2 text-sm font-semibold text-blue-600 border-b-2 border-blue-600 -mb-px transition"
  defp tab_class(false), do: "px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-700 border-b-2 border-transparent -mb-px transition"
